...
fn initialize_parameters(state: &mut State, params: Vec<Param>, body: &Statement) {
    for p in params {
        ...
        let address_instruction = ValueBucket {
            id: new_id(),
            source_file: state.files.get_filename_or_default(&body.get_meta().get_file_id()),
            line: 0,
            message_id: 0,
...
fn initialize_constants(state: &mut State, constants: Vec<Argument>, stmt: &Statement) {
    for arg in constants {
        ...
        let address_instruction = ValueBucket {
            id: new_id(),
            source_file: state.files.get_filename_or_default(&stmt.get_meta().get_file_id()),
            line: 0,
            message_id: 0,
            parse_as: ValueType::U32,
            value: address,
...
fn translate_if_then_else(stmt: Statement, state: &mut State, context: &Context) {
    ...
    if let IfThenElse { meta, cond, if_case, else_case, .. } = stmt {
        ...
        let branch_instruction = BranchBucket {
            id: new_id(),
            source_file: state.files.get_filename_or_default(&meta.get_file_id()),
            line: starts_at,
            message_id: state.message_id,
    ...
    if symbol.dimensions.is_empty() {
        ...
        for instruction in index_stack {
            ...
            let inst = ValueBucket {
                id: new_id(),
                source_file: at.get_source_file().to_string(),
                line: at.get_line(),
        ...
        match *i {
            ...
            op => {
                let to_address = ComputeBucket {
                    id: new_id(),
                    source_file: op.get_source_file().to_string(),
                    line: op.get_line(),
...
fn fold(using: OperatorType, mut stack: Vec<InstructionPointer>, state: &State, meta: &Meta) -> InstructionPointer {
    ...
    if stack.len() == 0 {
        ...
        ComputeBucket {
            id: new_id(),
            source_file: instruction.get_source_file().to_string(),
